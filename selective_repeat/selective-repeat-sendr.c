...
struct itimerval value; struct frame fr;
...
void handler(int sig) {
    ...
    switch (sig) {
        case SIGIO:
            ...
            ackno = f.header.ack; nakno = f.header.nak;
            if ((nakno > -1) && (ackno == -1)) {
                printf("Received NAK = %d\t", nakno); f.header.seq = nakno;
                f.header.ack = -1; f.header.nak = -1;
                printf("Sending repeat frame %d\n", nakno); send(sock, &f, sizeof(f), 0);
                value.it_value.tv_sec = TIMEOUT; setitimer (ITIMER_REAL, &value, 0);
                sleep(1);
            } else {
                printf("ACK %d received\t", ackno); slast = ackno;
            }
            break;
...
int main(int argc, char **argv){
    struct sockaddr_in recvr; int flags, noise;
    ...
    window = atoi(argv[2]); threshold = atoi(argv[3]);
    bzero(&recvr, sizeof(recvr)); recvr.sin_family = AF_INET;
    ...
    sigaddset(&ioact.sa_mask, SIGALRM); sigaction (SIGIO, &ioact, NULL);
    alrmact.sa_handler = handler; sigemptyset(&alrmact.sa_mask);
    sigaddset(&alrmact.sa_mask, SIGIO); sigaction (SIGALRM, &alrmact, NULL);
    intact.sa_handler = handler; sigemptyset(&intact.sa_mask);
